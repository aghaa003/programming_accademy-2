...
class CourseController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $courses = $query->get()->map(function ($course) {
            ...
            if (!empty($course->logo_path)) {
                $lp = str_replace('\\', '/', $course->logo_path);
                $lp = ltrim($lp, '/');
                // Logos are stored in the frontend project, not the Laravel project
                $frontendFile = realpath(base_path('../programming-academy/' . $lp));
                if ($frontendFile && file_exists($frontendFile)) {
                    $logoPath = 'http://localhost/programming-academy/' . $lp;
                }
            }
    ...
    public function userCourses(Request $request)
    {
        ...
        $courses = array_map(function ($row) {
            ...
            if (!empty($row['logo_path'])) {
                $lp = str_replace('\\', '/', $row['logo_path']);
                $lp = ltrim($lp, '/');
                $frontendFile = realpath(base_path('../programming-academy/' . $lp));
                if ($frontendFile && file_exists($frontendFile)) {
                    $row['logo_path'] = 'http://localhost/programming-academy/' . $lp;
                }
            }
    ...
    {
        $t = strtolower($title);
        if (str_contains($t, 'javascript') || str_contains($t, 'js')) return 'fab fa-js-square';
        if (str_contains($t, 'python')) return 'fab fa-python';
        if (str_contains($t, 'php')) return 'fab fa-php';
        if (str_contains($t, 'html')) return 'fab fa-html5';
        if (str_contains($t, 'css')) return 'fab fa-css3-alt';
        if (str_contains($t, 'react')) return 'fab fa-react';
        if (str_contains($t, 'c++')) return 'fas fa-code';
        if (str_contains($t, 'java')) return 'fab fa-java';
        if (str_contains($t, 'c#') || str_contains($t, 'csharp')) return 'fas fa-code';
        if (str_contains($t, 'sql')) return 'fas fa-database';
        return 'fas fa-code';
    }
    ...
    {
        $t = strtolower($title);
        if (str_contains($t, 'c++')) return 'cpp-icon';
        if (str_contains($t, 'python')) return 'python-icon';
        if (str_contains($t, 'php')) return 'php-icon';
        if (str_contains($t, 'javascript') || str_contains($t, 'js')) return 'js-icon';
        if (str_contains($t, 'html')) return 'web-icon';
        if (str_contains($t, 'css')) return 'web-icon';
        if (str_contains($t, 'react')) return 'react-icon';
        if (str_contains($t, 'java')) return 'java-icon';
        if (str_contains($t, 'c#') || str_contains($t, 'csharp')) return 'csharp-icon';
        if (str_contains($t, 'sql')) return 'sql-icon';
        return 'default-icon';
    }
}
