...
class VideoStreamController extends Controller
{
    ...
    public function stream(Request $request)
    {
        ...
        $lesson = Lesson::select('video_data', 'video_mime', 'title')->find($lessonId);
        if (!$lesson) {
            return response('Video not found', 404);
        }

        // Build full path - video_data stores relative path like videos/...
        $relativePath = str_replace('\\', '/', $lesson->video_data);
        $videoPath = realpath(base_path('../programming-academy/' . $relativePath));

        // Security: ensure path is within the original project videos directory
        $basePath = realpath(base_path('../programming-academy/videos'));
